...
export const Header: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  const adminNavigation = [
    ...
    { name: "Команды", href: "/team-panel-admin", current: false },
  ];

  const userNavigation = [
    { name: "Главная", href: "/", current: true },
    { name: "Тест", href: "/test", current: false },
    { name: "Моя команда", href: "/team-panel", current: false },
  ];
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="flex h-24 items-center justify-between">
                  ...
                  <div className="ml-10 flex items-baseline space-x-4">
                    {status == "loading"
                      ? userNavigation?.map((item) => (
                        <Link
                          key={item.name}
                          href={item.href.includes("home") ? "/public" : item.href}
                          className={cn(
              ...
              <div className="-mr-2 flex md:hidden">
                <DisclosureButton className="group size-14 relative  inline-flex items-center justify-center rounded-md  text-gray-400 hover:text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800">
                  <img
                    className="h-full"
                    src=".././free-icon-menu.png"
                    alt=""
                  />
                </DisclosureButton>
              </div>
